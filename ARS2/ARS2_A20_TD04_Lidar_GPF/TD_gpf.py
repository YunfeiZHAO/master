...
def naive_ground_extractor(ptc, n_lpr):
    ...
    # TODO
    sorted_ids = np.argsort(ptc[:, 2])
    ground_ids = sorted_ids[: n_lpr]
    return ground_ids
...
def gpf_extractinitialseeds(ptc, n_lpr, thresh_seeds = 0.5):
    ...
    sorted_ids = np.argsort(ptc[:, 2])
    ground_ids = sorted_ids[: n_lpr]
    lpr_pts = ptc[ground_ids, :3]
    lpr = np.mean(lpr_pts, axis=0)
    seeds_ids = np.where(ptc[:,2] < lpr[2] + thresh_seeds)
    return seeds_ids
...
def gpf_estimateplane(ptc, plane_ids):
    ...
    # TODO
    plane_pts= ptc[plane_ids[:], :3].reshape((-1, 3))
    s_hat = np.mean(plane_pts, axis=0)
    s_i = plane_pts - s_hat
    covar = np.matmul(s_i[0].reshape((3,1)), s_i[0].reshape((1,3)))
    for i in range(1, plane_pts.shape[0]):
        covar += np.matmul(s_i[0].reshape((3,1)), s_i[0].reshape((1,3)))
    u, s, _ = np.linalg.svd(covar)
    normal = u[:, np.argmin(s)]
    d = - np.matmul(normal, s_hat.reshape((3, 1))).flatten()
    return normal, d
...
def gpf_refinement(ptc, seeds_ids, thresh_dist=0.2, n_iter=5):
    ...
    for i in range(n_iter):
        normal, d = gpf_estimateplane(ptc, seeds_ids)
        distance = np.matmul(normal, ptc[:, :3].transpose()).flatten() + d
        seeds_ids = np.where(distance < thresh_dist)
...
def main(pcd_file):
    ...
    #ground_ids = naive_ground_extractor(ptc, 18000)
